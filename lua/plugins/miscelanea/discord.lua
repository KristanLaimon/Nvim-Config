...
local function hot_reload()
  package.loaded["plugins.miscelanea.discord"] = nil
  package.loaded["cord"] = nil
  package.loaded["cord.api.config"] = nil

  local ok_spec, spec = pcall(require, "plugins.miscelanea.discord")
  ...
  if ok_cord and cord.setup then
    ...
    vim.notify("🎮 Discord plugin (cord.nvim) hot-reloaded successfully!", vim.log.levels.INFO, { title = "Discord Plugin" })
  else
    vim.notify("⚠️ cord.nvim module not found, updated spec only", vim.log.levels.WARN, { title = "Discord Plugin" })
  end
end
