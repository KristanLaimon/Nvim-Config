...
**Through the picker (recommended):** open the target project in Neovim, run `:KrsTypes` (or `:TypeInjector`) from a `.ts`/`.js`/`.tsx`/`.jsx` buffer, toggle the schema on. This writes `.krsnvim/types.json`, regenerates `.krsnvim/types.d.ts` (one `/// <reference path>` per active schema), patches `tsconfig.json`'s `include` if needed, and notifies the running `tsgo` client — no restart.
...
```jsonc
// tsconfig.json -- must include the generated file, or tsgo ignores it entirely
...
Any project with the schema active picks it up the next time `tsgo` re-reads the referenced files (usually automatic on save; `:LspRestart` if not).
