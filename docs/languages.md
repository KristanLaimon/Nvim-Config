...
|---|---|---|
| [`krs.dap_repl_source`](debug-adapters.md#36-repl-completion-immediate-window) | `dap-repl`, only while a session is live | Real variables from the stopped frame, via the adapter's `completions` / `scopes` requests |
